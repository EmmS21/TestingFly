...
import { list_all_orders, update_order_status, get_item_order } from '../../redux/actions/orders'
import {
  get_items,
  get_total,
  get_item_total,
} from "../../redux/actions/cart";
import { get_search_order } from '../../redux/actions/orders';
...
import { SearchIcon, UploadIcon, CheckIcon, ClockIcon } from '@heroicons/react/solid'
...
const DashboardEmploy = ({
  ...
  const changeStatus = (TrID, Status) => async => {
    ...
    Swal.fire({
      ...
      .then(resultado => {
        if (resultado.value) {
          if (Status == "no_procesado") {
            update_order_status(TrID, "procesado")
            setTimeout('document.location.reload()', 1000);
          }
          if (Status == "procesado") {
            update_order_status(TrID, "enviado")
            setTimeout('document.location.reload()', 1000);
          }
        } else {
          // Dijeron que no
  ...
  function Alert(product) {
    if (product.status == "no_procesado" || product.status == "procesado") {

      return (
        <>

          <div
            className='ml-36 font-medium text-red-600 hover:text-red-500 cursor-pointer mt-4 sm:mt-0'>
  ...
  function action(product) {
    if (product.status == "no_procesado") {
      return (
        <>
          <div className="mt-4 sm:mt-0">
            <div className="  rounded-md bg-indigo-600  font-medium text-white hover:bg-indigo-500 cursor-pointer mt-4 sm:mt-0">
              <button
                className='p-0.5'
                onClick={changeStatus(product.transaction_id, product.status)}>Pedido Procesado</button>
            </div>
          </div>
        </>
      )
          ...
          <>
            <div className="mt-4 sm:mt-0">
              <div className="rounded-md bg-indigo-600  font-medium text-white hover:bg-indigo-500 cursor-pointer mt-4 sm:mt-0">
                <button
                  className='p-0.5'
                  onClick={changeStatus(product.transaction_id, product.status)}>Pedido Enviado</button>
              </div>
            </div>
          </>
        )
            ...
            <>
              <div className="mt-4 sm:mt-0">
                <div className="font-medium text-green-600 ">
                  Se envió este pedido
                </div>
              </div>
            </>
          )
        } else {
          if (product.status == "cancelada") {
            return (
              <>
                <div className="mt-4 sm:mt-0">
                  <div className="font-medium text-red-600 ">
                    Este pedido fué cancelado
                  </div>
                </div>
              </>
            )
  ...
  if (render) {
    return <Navigate to='/searchOrder' />;
  }
      ...
      <div>
        ...
        <div className="md:pl-64 flex flex-col flex-1">
          <div className="sticky top-0 z-10 flex-shrink-0 flex h-16 bg-white shadow">
            ...
            <div className="flex-1 px-4 flex justify-between">
              <div className="flex-1 flex">
                <form className="w-full flex md:ml-0" onSubmit={e => onSubmit(e)}>
          ...
          <main className="flex-1">
            <h1 className="text-3xl font-extrabold tracking-tight text-gray-900 ml-12 mt-7 mb-2 ">
              Ordenes</h1>
            <hr />
            <div className="">
                        ...
                        <div className="space-y-12 ">
                          {orders && orders.map((product) => (
                            ...
                            < >
                              ...
                              <div>
                                <div>
                                  <h1 className="text-3xl font-extrabold tracking-tight text-gray-900 mt-12">{product.full_name}</h1>
                                  <dt className="font-medium text-gray-900">Contacto</dt>
                                  <dd className=" text-gray-500">
                                    <span className="block">{product.telephone_number}</span>
                                  </dd>
                                </div>
                              </div>
                              <div className="text-sm border-b border-gray-200 mt-2 pb-5 sm:flex sm:justify-between">
                                <dl className="flex">
                                  ...
                                  <dd className="truncate font-medium text-gray-900 w-36">{product.transaction_id}</dd>
                                  <dt>
                                  ...
                                  <dl className="grid grid-cols-1 gap-y-8 border-b py-8 border-gray-200 sm:grid-cols-2 sm:gap-x-6 sm:py-6 md:py-10">
                                    <div>
                                      ...
                                      <dd className="mt-3 text-gray-500">
                                        ...
                                        <span className="block">{product.city}</span>
                                        <span className="block">{product.state_province_region}</span>
                                      </dd>
                                    </div>
                                    <div>
